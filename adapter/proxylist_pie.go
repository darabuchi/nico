...
func (ss ProxyList) DropTop(n int) (drop ProxyList) {
	...
	drop = make([]AdapterProxy, len(ss)-n)
	copy(drop, ss[n:])
...
func (ss ProxyList) Mode() ProxyList {
	...
	values := make(map[AdapterProxy]int)
	for _, s := range ss {
		values[s]++
	}
...
func (ss ProxyList) Reverse() ProxyList {
	...
	sorted := make([]AdapterProxy, len(ss))
	for i := 0; i < len(ss); i++ {
...
func (ss ProxyList) SubSlice(start int, end int) (subSlice ProxyList) {
	...
	if start < length {
		if end <= length {
			...
			zeroArray := make([]AdapterProxy, end-length)
			subSlice = ss[start:length].Append(zeroArray[:]...)
		}
	} else {
		zeroArray := make([]AdapterProxy, end-start)
		subSlice = zeroArray[:]
	}
